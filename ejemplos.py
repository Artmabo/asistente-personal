...
def ejemplo_1_limpieza_basica():
    ...
    resumen = limpiar_correos(dry_run=True)
...
def ejemplo_2_limpieza_personalizada():
    ...
    resumen = limpiar_correos(service=service, meses=3, solo_no_leidos=False, dry_run=True)
    print(f"Resultado: {resumen}\n")
...
def ejemplo_5_limpieza_agresiva():
    ...
    service = get_gmail_service()
    resumen = limpiar_correos(service=service, meses=12, solo_no_leidos=False, dry_run=True)
    print(f"Resultado: {resumen}\n")
...
if __name__ == "__main__":
    ...
    # Todos los ejemplos de limpieza corren en dry_run=True por defecto;
    # pasa dry_run=False explícitamente para borrar correos de verdad.
