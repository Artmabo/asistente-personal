...
from .utils import get_header, mask_email
...
class GmailProcessor:
    ...
    def _apply(self, msg_id: str, message: dict, c: Classification):
        ...
        # Sender is masked because this is logged at INFO (the default level,
        # written to a plaintext, non-rotated log file) — avoids persisting
        # correspondents' full addresses on disk.
        logger.info(
            f"[{c.email_type.upper():<12}] {_short(mask_email(sender), 40)} | {_short(subject, 50)}"
